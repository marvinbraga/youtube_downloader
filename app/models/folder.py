...
class FolderCreate(BaseModel):
    ...
    parent_id: Optional[str] = Field(
        None, description="ID da pasta pai (None para raiz)"
    )
    description: Optional[str] = Field(None, description="Descrição da pasta")
    color: Optional[str] = Field(
        None, pattern=r"^#[0-9A-Fa-f]{6}$", description="Cor em hex (ex: #FF5733)"
    )
...
class FolderUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=1, max_length=255, description="Nome da pasta"
    )
    parent_id: Optional[str] = Field(None, description="ID da pasta pai")
    description: Optional[str] = Field(None, description="Descrição da pasta")
    color: Optional[str] = Field(
        None, pattern=r"^#[0-9A-Fa-f]{6}$", description="Cor em hex"
    )
    icon: Optional[str] = Field(None, max_length=50, description="Ícone da pasta")
...
class MoveItemRequest(BaseModel):
    ...
    folder_id: Optional[str] = Field(
        None, description="ID da pasta destino (None para raiz)"
    )
